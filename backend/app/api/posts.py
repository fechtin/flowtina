...
def list_posts(
    ...
    items = PostService(db).list(
        project.id, status=status, language=language, keyword=keyword, limit=limit, offset=offset
    )
...
def create_post(
    payload: PostCreate, project: Project = Depends(get_owned_project), db: Session = Depends(get_db)
):
