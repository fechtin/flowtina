...
class Post(Base, BaseModelMixin):
    ...
    status: Mapped[str] = mapped_column(
        String(16), default="draft", index=True, nullable=False
    )
    ...
    # Optional attached image. ``image_url`` is a public URL handed directly to
    # the Graph API; ``image_path`` is a locally uploaded file (relative to the
    # configured upload dir) streamed as binary and removed after a successful
    # publish. At most one is set per post.
...
class PostVersion(Base, BaseModelMixin):
    ...
    post_id: Mapped[str] = mapped_column(
        String(36), ForeignKey("posts.id"), index=True, nullable=False
    )
...
class SchedulerJob(Base, BaseModelMixin):
    ...
    job_type: Mapped[str] = mapped_column(
        String(32), default="generate_content", nullable=False
    )
    ...
    content_type: Mapped[str] = mapped_column(
        String(32), default="short_post", nullable=False
    )
    language: Mapped[str] = mapped_column(String(8), default="en", nullable=False)
